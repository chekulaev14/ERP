...
import {
  ...
  emptyItemFormValues,
  type ItemFormValues,
} from '@/components/warehouse/ItemForm';
...
export function NomenclatureTab({ items, balances }: Props) {
  ...
  const fetchPotential = useCallback(() => {
    api
      .get<{ items: PotentialItem[] }>('/api/stock/potential', { silent: true })
      .then((d) => {
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-border hover:bg-transparent">
                    <TableHead className="text-muted-foreground text-sm font-medium h-8 pl-10">
                      Наименование
                    </TableHead>
                    <TableHead className="text-muted-foreground text-sm font-medium h-8 w-20 text-right">
                      Остаток
                    </TableHead>
                    <TableHead className="text-muted-foreground text-sm font-medium h-8 w-24 text-right">
                      Можно произвести
                    </TableHead>
                    <TableHead className="text-muted-foreground text-sm font-medium h-8 w-12 text-right">
                      Ед.
                    </TableHead>
                  </TableRow>
                </TableHeader>
                    ...
                    >
                      ...
                      <TableCell className="py-2 text-right">
                        {item.type !== 'material' && potentialMap[item.id] !== undefined ? (
                          <span
                            className={`text-sm font-mono ${potentialMap[item.id] > 0 ? 'text-emerald-600' : 'text-muted-foreground'}`}
                          >
                            {formatNumber(potentialMap[item.id])}
                            {potentialMap[item.id] === 0 && bottleneckMap[item.id] && (
                              <span
                                className="text-red-400 text-[10px] font-sans font-normal ml-1"
                                title={`Не хватает: ${bottleneckMap[item.id]}`}
                              >
