...
interface CatalogProduct {
  ...
  description: string;
  images: string[];
  parts: { id: string; name: string; description: string; images: string[]; pricePerUnit: number }[];
}
...
export async function getCatalog(): Promise<CatalogCategory[]> {
  ...
  const bomMap = new Map<string, CatalogProduct["parts"]>();
  ...
  const mapProduct = (p: typeof products[number]): CatalogProduct => ({
    id: p.id,
    name: p.name,
  ...
  const categories = dbCategories
    .map((cat) => ({
      ...
      products: products.filter((p) => p.categoryId === cat.id).map(mapProduct),
    }))
    .filter((cat) => cat.products.length > 0);

  const uncategorized = products.filter((p) => !p.categoryId).map(mapProduct);
